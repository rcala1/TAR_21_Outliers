from utils_deep import QuoraDataBert, train_deep,evaluate_val,evaluate_test,extract_new_examples_idxs_bert
...
X_dataset, y_dataset = utils_general.get_Xy_train(utils_general.load_dataset_numpy('/content/gdrive/MyDrive/Outliers/train.csv'))
...
experiment_combinations=itertools.product(initial_percentages,increasing_percentages,methods)
...
for init_perc,incr_perc,method in experiment_combinations:
  ...
  bert_log.write("BERT Initial Percentage {} Increasing Percentage {} Method {}\n".format(init_perc,incr_perc,method))
  ...
  while(True):
    quora_dataset = QuoraDataBert([X_train_current,X_train_pooling],[y_train_current,y_train_pooling])
    train_current_loader,train_pooling_loader=quora_dataset.get_datasets_loaders(shuffle=False)
    ...
    bert_log.write("Train Examples {}, Pooling Examples {}, Val Acc {}, Test Acc {}\n"
                    .format(len(X_train_current),len(X_train_pooling),val_acc,test_acc))
    bert_log.flush()

    if len(X_train)==len(X_train_current): break

    new_indexes=extract_new_examples_idxs_bert(model,train_pooling_loader,increasing_number,device,method)
